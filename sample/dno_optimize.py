...
from motion_optimizer.motion_optimizer import Motion2DOptimizer
...
def main():
    ...
    max_frames = 999 if args.dataset in ['kit', 'humanml'] else 60
    ...
    # Load the target motion
    target_motion = torch.ones((args.batch_size, model.njoints, model.nfeats, n_frames), device=dist_util.dev())
    
    for rep_i in range(args.num_repetitions):
        
        def gen_motion(xt, all_text):
            ...
            all_motions.append(sample.clone().detach().cpu().numpy())
            all_lengths.append(model_kwargs['y']['lengths'].cpu().numpy())

            print(f"created {len(all_motions) * args.batch_size} samples")
            return sample, model_kwargs['y']['lengths'], model_kwargs['y']['mask']
            
        print(f'### Sampling [repetitions #{rep_i}]')
        # DNO-loop
        xt = torch.randn((args.batch_size, model.njoints, model.nfeats, max_frames), device=args.device) # Being optimize
        motion_optimizer = Motion2DOptimizer(xt, lr=1e-2, thres=1e-1, device=args.device)
        while motion_optimizer.is_converged():
            motion_3d, motion_length, motion_mask = gen_motion(motion_optimizer.params['xt'], all_text)
            loss = motion_optimizer.forward(motion_3d, target_motion)
            print(f'Loss: {loss}')
            print(torch.mean(motion_optimizer.params['xt']))
            print(motion_optimizer.params['distance'])
